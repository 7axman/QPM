...
# --- Configuration ---
INSTALL_DIR="$HOME/~DIY-Scripts/QPM"
VENV_DIR="$INSTALL_DIR/venv"
PLIST_DIR="$HOME/Library/LaunchAgents"
PLIST_FILENAME="com.user.qpmupdater.plist"
...
echo_bold "=========================================="
echo_bold "                   QPM                    "
echo_bold "    qBittorrent ProtonVPN Manager (Mac)   "
echo_bold "=========================================="
echo ""

# 1. Smart Detection
if [ -f "$PLIST_PATH" ]; then
    echo_yellow "QPM is already installed."
...
# 2. Introduction
echo "This script automates Proton VPN port forwarding and interface binding for qBittorrent on macOS."
...
class PortUpdater:
    ...
    def _get_sid(self):
        try:
            ...
            if resp.ok and resp.cookies: return resp.cookies
            logging.error(f"qBt auth failed. Status: {resp.status_code}")
        except Exception as e: logging.error(f"qBt auth failed: {e}")
        return None

    def _get_vpn_interface(self):
        try:
            ...
            for line in output.split('\n'):
                ...
                if 'inet 10.2.' in line and current_iface:
                    return current_iface
        except Exception as e:
            logging.error(f"Failed to get vpn interface: {e}")
        return None

    def _update_qbt_port(self, cookies, port, interface=None):
        try:
            prefs = {'listen_port': port}
            if interface:
                prefs['current_network_interface'] = interface
            requests.post(f"{self.config['qbt_url']}/api/v2/app/setPreferences", cookies=cookies, data={'json': json.dumps(prefs)}, timeout=10)
            logging.info(f"Updated qBittorrent port to {port}" + (f" and interface to {interface}" if interface else ""))
            return True
    ...
    def run(self):
        ...
        try:
            while not self.stop_event.is_set():
                ...
                current_iface = self._get_vpn_interface()
                
                if port_to_check and (port_to_check != self.last_synced_port or current_iface != self.last_synced_iface):
                    if sid := self._get_sid():
                        if self._update_qbt_port(sid, port_to_check, current_iface): 
                            self.last_synced_port = port_to_check
                            self.last_synced_iface = current_iface
...
<plist version="1.0">
<dict>
    <key>Label</key><string>com.user.qpmupdater</string>
...
echo_bold "=========================================="
echo ""
echo "The QPM service is now running in the background."
echo "Log file: ~/~DIY-Scripts/QPM/updater.log"
echo ""
